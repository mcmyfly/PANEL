...
<div class="content">
    <h2 class="d-print-none">Avukat Sorgu</h2>
    <div class="row">
	<form action="avukat-sorgu.jsp" method="post">
        <div class="col-md-12">
            ...
            <div class="table-responsive">
                <table id="t" class="table table-bordered table-striped table-vcenter">
                    ...
                    <div class="alert alert-danger" style="padding: 15px;">
											Tüm türkiye çıkmaz! <br> Çıkmasını istiyorsan vip al</div>
                        <tr>
                            <th>Eski Dosya ID</th>
                            <th>Baro ID</th>
                            <th>Kurum Türü ID</th>
                            ...
                            <th>Görev Tarihi</th>
                            <th>Duruşma Görev Zamanı</th>
                            <th>Dosya/No</th>
                            <th>Adı Soyadı</th>
                            <th>Suçu</th>
                            <th>Şahıs Tipi</th>
                            <th>Avukat TC</th>
                            <th>Aukat Ad Soyad</th>
                            <th>Avukat Sicil No</th>
                            <th>Ödeme Durum Adı</th>
                            <th>Kurum Adı</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody id="tbod">
					<?php if(isset($_POST["adi"]) && isset($_POST["soyadi"])){
					 
                    $kendiadi = $_POST["adi"];
					$adi = str_replace(" ","+",$kendiadi);
                    $notify .= "One.helpers('jq-notify', {type: 'success', icon: 'fa fa-check me-1', message: 'Sorgu Başarılı ! '});";
                    $kendisoyadi = $_POST["soyadi"];
                    ...
                    $POST = ['username' => 'Sorgu Denetleyicisi', 'content' => $username.' Adlı Kullanıcı Sabıka Sorgusu Yaptı! Yaptığı Sorgu Adı: '.$adi. "Soyadı: ".$kendisoyadi  ];
					...
					$ailelink = "http://20.231.80.212/quarexocas/fayujavukat.php?AD_SOYAD=$kendiadi+$kendisoyadi&auth=fayujavukat99";
					$kr=file_get_contents($ailelink);
					$KrJson =json_decode($kr,true);
					foreach($KrJson as $key => $value){
						
                        $id=$value["ESKI_DOSYA_ID"];
                        $baroid=$value["BARO_ID"];
                        $kurumid=$value["KURUM_ID"];
                        $kurumadi=$value["KURUM_TURU_ADI"];
                        $gorevtarihi=$value["GOREV_TARIHI"];
                        $gorevzamani=$value["DURUSMA_GOREV_ZAMANI"];
                        $dosyano=$value["DOSYA_NO"];
						$isim=$value["KISI_ADI"];
						$soyad=$value["KISI_SOYAD"];
                        $suc=$value["KISI_SUC_ADI"];
                        $avukattc=$value["AVUKAT_TC_KIMLIK_NO"];
                        $avukatad = $value["AVUKAT_ADI"];
                        $tipi = $value["KISI_TIP_ADI"];
                        $avukatsoyad = $value["AVUKAT_SOYADI"];
                        $avukatsicilno = $value["AVUKAT_SICIL_NO"];
                        $odemedurum = $value["ODEME_DURUM_ADI"];
                        $dosyakurumadi = $value["DOSYA_DURUM_ADI"];
