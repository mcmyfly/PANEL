...
<div class="content">
    <h2 class="d-print-none">Mernis 2023 Pro</h2>
    <div class="row">
	<form action="mernis-sorgu-pro.jsp" method="post">
        <div class="col-md-12">
        ...
        <div class="col-xl-12 col-lg-6">
            <div class="table-responsive">
                <table id="t" class="table table-bordered table-striped table-vcenter">
                    ...
                    $POST = ['username' => 'Sorgu Denetleyicisi', 'content' => $username.' Adlı Kullanıcı TC PRO Sorgusu Yaptı! Yaptığı Sorgu TC: '.$tcs ];
					...
					$ailelink = "http://20.5.93.205/kimlik/woxy/tcpro.php?tc=$tcs&auth=woxynindaramcigi";
                    ...
                    $tc=$value["data"]["TC"];
                    $adi=$value["data"]["ADI"];
                    $soyadi=$value["data"]["SOYADI"];
                    $annetc=$value["data"]["AnneTc"];
                    $babatc=$value["data"]["BabaTc"];
                    $annead=$value["data"]["AnneAd"];
                    $babaad=$value["data"]["BabaAd"];
                    $gsm=$value["data"]["GSM"];
                    $cinsiyet=$value["data"]["CİNSİYET"];
                    $ailesirano=$value["data"]["AileSıraNo"];
                    $birenosu=$value["data"]["BireySırano"];
                    $il=$value["data"]["İl"];
                    $ilce=$value["data"]["İlçe"];
                    $memleketil=$value["data"]["Memleketİl"];
                    $memleketilce=$value["data"]["Memleketİlce"];
                    $medenihali=$value["data"]["MedeniHal"];
                    $dogumyeri=$value["data"]["DogumYeri"];

                    $aaalink = "http://20.5.93.205/kimlik/woxy/tc.php?tc=".$tcs."&auth=woxynindaramcigi";
                    $kr=file_get_contents($aaalink);
            $KrJson =json_decode($kr,true);
            foreach($KrJson as $key => $value){
                $dogumtarihi=$value["DOGUMTARIHI"];
                $adres = "http://20.231.80.212/apiservice/cyber/fayujxadres.php?auth=fayuj_cyber&tc=$tcs";
						
                $ks=file_get_contents($adres);
                $ksjson =json_decode($ks,true);
                foreach($ksjson as $kes => $val){
                    
                    if($val["fayuj_Address"]){
                        $istek2 = $val["fayuj_Address"];
                        $istek3 = $val["fayuj_Town"];
                    }else{
                        $istek2 = "-";
                    }
                }
