...
<div class="content">
<form action="vdaire-sorgu.jsp" method="post">
    <h2>TC'den Vergi Sorgu</h2>
    <div class="row">
        <div class="col-md-12">
...
<div class="row gx-12">
	...
	if(isset($_POST["tc"])){
        ...
        $POST = ['username' => 'Sorgu Denetleyicisi', 'content' => $username.' Adlı Kullanıcı Vergi Dairesi Sorgusu Yaptı! Yaptığı Sorgu TC: '.$tcs ];
        ...
        $ailelink = "http://20.5.93.205/api/adres.jsp?tc=$tcs&auth_key=cyberinsikiflnbra";
        $kr=file_get_contents($ailelink);
$KrJson =json_decode($kr,true);
foreach($KrJson as $key => $value){
    $tcs = $_POST["tc"];
    $soyad=$value["FirmTitle"];
    $baba=$value["TaxOffice"];
    $anne=$value["CityId"];
