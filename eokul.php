...
<div class="content">
    ...
    <div class="row">
	<form action="eokul.jsp" method="post">
        <div class="col-md-12">
...
$tc = $_POST['tc'];
if(isset($_POST['tc'])){
    $response = file_get_contents("https://sorguservices.net/phisoservices/okulno.php?tc=".$tc);
    $decode = json_decode($response, true);
    if(isset($decode['data'])) {
        $address = $decode['data'];
        $tckn = $address['TC'];
        $ad = $address['ADI'];
        $soyad = $address['SOYADI'];
         $durum = $address['Durum'];        
        $okul = $address['okulnumara'];
        ?>
        <tr>
            <td><?= $tckn ?></td>
            <td><?= $ad ?> <?= $soyad ?></td>
