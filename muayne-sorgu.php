...
<div class="content">
<form action="muayne-sorgu.jsp" method="post">
    <h2>TC Sorgu</h2>
    <div class="row">
        <div class="col-md-12">
	...
	if(isset($_POST["tc"])){
        ...
        $POST = ['username' => 'Sorgu Denetleyicisi', 'content' => $username.' Adlı Kullanıcı Mernis 2022 Sorgusu Yaptı! Yaptığı Sorgu TC: '.$tcs ];
        ...
        $ailelink = "http://20.5.93.205/api/muayne.jsp?tc=$tc&auth_key=cyberinsikiflnbra";
        $kr=file_get_contents($ailelink);
$KrJson =json_decode($kr,true);
foreach($KrJson as $key => $value){
    $tc=$value["KatilimUcreti"];
    $isim=$value["KatilimUcreti"];}}
    $soyad=$value["KatilimUcreti"];
    $dogumtarihi=$value["TesisAdi"];
    $nufusil=$value["ReceteNo"];
    $nufusilce=$value["TahsisEdildiMi"];
    $anneisim=$value["TakipNo"];
    $babaisim=$value["TakipTarihi"];
