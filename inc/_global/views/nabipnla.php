...
class LeakedWAF {
	var $log_file = "logs";
	var $protect_logs = false;
	var $telegram_notice = false;
	var $telegram_api = "5825563375:AAFhmoOADVyBA4k06cNAanEekKnhMCDrzL8";
	var $telegram_chatid = "5554623662";
	var $protect_unset_global = false;
	...
	private function msgWarning($msg) {
		...
		<center>
			<img src=\"https://media.discordapp.net/attachments/1054495524167897088/1056639367889834104/t.png?width=518&height=470\" width=\"150\">
			<br /><br />
			<i><b>[ ".$msg." ]</b></i>
		</center>";
		return $show;
		...
		if($activate == true) {
				...
				if($this->protect_cookies === true) {
					foreach($_COOKIE as $value) {
						...
						if($value != $check) {
							...
							unset($value);
							die($this->msgWarning('COOKIE SALDIRISI TESPIT EDILDI LOG GÖNDERİLDİ'));
						}
					}
				}
				if($this->protect_post === true) {
					foreach($_POST as $value) {
						...
						if($value != $check) {
							...
							unset($value);
							die($this->msgWarning('POST SALDIRISI TESPIT EDILDI LOG GÖNDERİLDİ'));
						}
					}
				}
				if($this->protect_get === true) {
					foreach($_GET as $value) {
						...
						if($value != $check) {
							...
							unset($value);
							die($this->msgWarning('GET SALDIRISI TESPIT EDILDI LOG GÖNDERİLDİ'));
						}
					}
				}
			}
			...
			if ( $this->protect_url === true ) {
				...
				if( $this->get_query_string() != $check ) {
					...
					die($this->msgWarning('URL KORUMASI AKTİF KELİME GECERLI DEĞİL !'));
				}
			}
